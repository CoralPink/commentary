...
const compileScss = async (input, output) => {
	...
	try {
		...
		console.info(
			`[INFO]:   ${CLR_BC}sass${CLR_RESET} ${CLR_C}${fileName}${CLR_RESET}🎁 ${cssSize} byte`,
		);
...
(async () => {
	console.info(
		`[INFO]: 🎅 I'll use ${CLR_BC}sass${CLR_RESET} to create ${CLR_BG}css${CLR_RESET}!`,
	);
	...
	try {
		...
		const mainFiles = FILES.map((file) =>
			compileScss(`${file}.scss`, `${OUT_DIR}${file}.css`),
		);

		const themeFiles = THEME_FILES.map((file) =>
			compileScss(
				`${THEME_DIR}${file}.scss`,
				`${OUT_DIR}${THEME_DIR}${file}.css`,
			),
		);
	...
	console.info(
		`    ${CLR_BG}Finished${CLR_RESET} release target(s) in ${time}s 🧒${CLR_Y}Happy!!${CLR_RESET}`,
	);
})();
