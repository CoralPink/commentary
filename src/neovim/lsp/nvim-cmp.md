...
あらぬ誤解を招きそうな詩で始まってしまいますが、ついに Abbey Road {{footnote:
...
壮大なメドレー{{footnote: わたし自身は John Lennon が一番好きですが、Abbey Road はA面もB面も大好きです。アナログレコードで聴いたことはないんだけど😅}}はここから始まります❗
楽しんでいきましょう😆
...
今回は`nvim-cmp`のインストールとキーマッピングだけなので何も変化ありませんが、
これも慌てずどっしり構えておけば超だいじょうぶです😤

そして次回、`Language Server`を使用した補完を行います。
